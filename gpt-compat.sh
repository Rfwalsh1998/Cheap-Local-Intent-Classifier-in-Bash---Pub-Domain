#!/usr/bin/env bash
# GPT-tuned degen intent classifier v1
# Based on the Grok-tuned classifier from Rfwalsh1998 (Unlicense / public domain)
# Tweaks: balanced code gen, GPT-specific intents (image/search/tool),
...
hit image_generation 12 '(generate.*image|draw|create.*(picture|art|image)|imagine|visualize|render|picture of|photo of|dall.?e|dalle|image generation)' 'gpt_image_terms'
hit web_search 10 '(search (the )?(web|internet|google)|look up|search online|online search|find on the web|google this|look this up|search for)' 'gpt_search_terms'
hit tool_usage 9 '(use (your|the) (tools?|search|tool)|call (a )?tool|run code|execute code|use code interpreter|use browser)' 'tool_terms'
